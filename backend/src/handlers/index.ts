...
class MoviesHandler {
  ...
  async get(req: Request, res: Response) {
    try {
      ...
      if (getAllMovies.length === 0) return res.status(404).json({message: 'A movie is yet to be created.'})
  ...
  async update(req: Request, res: Response) {
    ...
    if (!likeMovie) return res.status(404).json({ message: 'Movie does not exist'});
